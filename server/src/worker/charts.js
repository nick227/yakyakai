export const CHART_TYPES_PROMPTS = [
    ...
    // MERMAID (process)
    'Use Mermaid to create a flow diagram using <pre class="mermaid">. Never use bare quoted strings as node IDs. Connect nodes logically. Do not invent data, only use sources from the web.',
  
    // TABLE (structured info)
    'Create a clean table with 3–5 rows and 2–4 columns showing structured information about the subject. Use real categories and meaningful values. Avoid placeholders like "Item 1". Ensure each row adds distinct value (no repetition). Do not invent data, only use sources from the web.',
